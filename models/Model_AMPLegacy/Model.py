    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run() or ask_override:
            if (self.read_from_conf and not self.config_file_exists) or not self.read_from_conf:
                ...
                self.options['clipgrad'] = io.input_bool ("用梯度裁剪 Enable gradient clipping", default_clipgrad, help_message="梯度裁剪降低了模型崩溃的几率，但牺牲了训练速度.")
                self.options['pretrain'] = io.input_bool ("启用预训练模式 Enable pretraining mode", default_pretrain, help_message="使用大量各种人脸预训练模型,模型可用于更快地训练伪造数据.强制使用 random_warp=N, random_flips=Y, gan_power=0.0, lr_dropout=N, uniform_yaw=Y")
